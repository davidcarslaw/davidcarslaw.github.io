...
{{% alert note %}}
The UK has a surprisingly large amount of air quality data available. The main national network is called the [Automatic Urban and Rural Network](https://uk-air.defra.gov.uk/networks/network-info?view=aurn). There are however other large networks including the [Scottish Air Quality Network](http://www.scottishairquality.scot/) and the [Welsh Air Quality Network](https://airquality.gov.wales/). One of the largest --- if not the largest urban air quality network is the [London Air Quality Network](https://www.londonair.org.uk/LondonAir/Default.aspx)
{{% /alert %}}

[openair]{.pkg} includes the function `importMeta` to provide information on UK air pollution monitoring sites. There are currently four networks that [openair]{.pkg} has access to:
...
Since [openair]{.pkg} started there have been huge developments with R and optional packages. These developments have made it much easier to manipulate and plot data e.g. with [ggplot2]{.pkg} and the likes of [dplyr]{.pkg}. There is also now much more focus on interactive plotting, which is very useful in the context of considering air pollution sites.
...
The information above should help to describe the air quality data that is easily available through [openair]{.pkg}. Access to the data is possible through  a family of functions that all tend to work in a simuilar way. Only two pieces of information are required: the site code(s) and the year(s) of interest.
...
This data also includes estimates of wind speed and direction (`ws` and `wd`) from the WRF model, so we camn easily plot the distribution of concentrations by wind direction. This plot indicates that the highest PM$_{10}$ concentrations are from the south-west i.e. the steelworks direction. A better indication of important steelworks combustion sources can be seen by plotting SO$_2$.
