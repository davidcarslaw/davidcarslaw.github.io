---
title: Analysis of long-term observations of NO$_x$ and CO in megacities and application to constraining emissions inventories
date: '2016-01-01'
author:
- B. Hassler
- B.C. McDonald
- G.J. Frost
- A. Borbon
- D.C. Carslaw
- K. Civerolo
- C. Granier
- P.S. Monks
- S. Monks
- D.D. Parrish
- I.B. Pollack
- K.H. Rosenlof
- T.B. Ryerson
- E. von Schneidemesser
- M. Trainer
categories:
  - research
  - NOx
  - emissions inventory
  - trend analysis
description: 'A fuel-based emission inventory approach applied to long-term NO$_x$/CO observations in megacities shows that vehicle emissions controls drove US urban NO$_x$/CO trends over 50 years, while contrasting European trends reflect slower progress due to diesel use, revealing limitations of commonly used global inventories in reproducing these changes.'
doi: 10.1002/2016GL069894
publication: '*Geophysical Research Letters*'
---

## Analysis of long-term observations of NO$_x$ and CO in megacities and application to constraining emissions inventories

<p class="pub-meta">B. Hassler, B.C. McDonald, G.J. Frost, A. Borbon, D.C. Carslaw, K. Civerolo, C. Granier, P.S. Monks, S. Monks, D.D. Parrish, I.B. Pollack, K.H. Rosenlof, T.B. Ryerson, E. von Schneidemesser, M. Trainer</p>

_*Geophysical Research Letters*_, 2016


<div class="pub-abstract">
Long-term atmospheric NO$_x$/CO enhancement ratios in megacities provide evaluations of emission inventories. A fuel-based emission inventory approach that diverges from conventional bottom-up inventory methods explains 1970–2015 trends in NO$_x$/CO enhancement ratios in Los Angeles. Combining this comparison with similar measurements in other U.S. cities demonstrates that motor vehicle emissions controls were largely responsible for U.S. urban NO$_x$/CO trends in the past half century. Differing NO$_x$/CO enhancement ratio trends in U.S. and European cities over the past 25 years highlights alternative strategies for mitigating transportation emissions, reflecting Europe's increased use of light-duty diesel vehicles and correspondingly slower decreases in NO$_x$ emissions compared to the U.S. A global inventory widely used by global chemistry models fails to capture these long-term trends and regional differences in U.S. and Europe megacity NO$_x$/CO enhancement ratios, possibly contributing to these models' inability to accurately reproduce observed long-term changes in tropospheric ozone.
</div>


<div class="pub-links">
<a href="https://doi.org/10.1002/2016GL069894" class="btn btn-primary btn-sm" target="_blank"><i class="bi bi-box-arrow-up-right"></i> DOI</a>
<a href="carslaw-2016.bib" class="btn btn-outline-secondary btn-sm"><i class="bi bi-download"></i> BibTeX</a>
</div>
